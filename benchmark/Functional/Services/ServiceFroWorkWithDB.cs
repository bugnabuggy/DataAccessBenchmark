...
namespace benchmark.Functional.Services
{
  public class ServiceFroWorkWithDB : IServiceFroWorkWithDB
  {
    ...
    public ServiceFroWorkWithDB(IRepository<Vendor> vendor, IRepository<WareHouse> wareHouse, IRepository<Product> product, IRepository<ProducteInWareHouse> producteInWareHouse, IRepository<TestHistory> repositoryHistory, IConfiguration configuration)
    {
      this._vendor = vendor;
    ...
    public string CheckRecordLimit(int countRecords)
    {
      int countRecordsInBaseData = this._vendor.GetAll().ToList().Count;
      int summRecords = countRecords + countRecordsInBaseData;
      string result = "ok";
      var limitRecords = Int32.Parse(this._configuration["RecordLimit"]);
      if (summRecords > limitRecords)
      {
        result = "Record limit exceeded.There can be no more than " + limitRecords + " records. In the moment " + countRecordsInBaseData + " records";
      }
      return result;
    }

    public string Fill(int countRecords)
    {
      string result = "";
      if ("ok" == CheckRecordLimit(countRecords))
      {
      ...
      {
        result = CheckRecordLimit(countRecords);
      }
      return result;
    }
